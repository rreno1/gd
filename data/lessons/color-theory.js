...
window.GDLessons["color-theory"] = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "The digital color wheel (RGB/HSB) organizes hues based on screen light. Red, Green, and Blue are spaced evenly at 0, 120, and 240 degrees around the wheel. The secondary colors — Cyan, Magenta, and Yellow — occupy the spaces directly opposite their primary complements. This model aligns with the physical mechanics of screens and is the default wheel found in design tools like Figma, Illustrator, and Photoshop. Understanding the digital wheel is crucial for web and UI designers who specify colors using hex codes, HSL, or RGB values.",
        "A color wheel is a tool for planning relationships, not a formula that guarantees success. Simply selecting colors that form a perfect geometric shape on the wheel (like a triangle for a triad) does not mean they will look good in a layout. The wheel tells you nothing about value contrast, saturation control, or visual weight. A successful palette uses the color wheel to discover hue relationships, but relies on design judgment to adjust lightness, saturation, and area proportions to suit the project's actual content and goals."
      ],
      keyPoints: [
    ...
    },
    {
      ...
      body: [
        "To work with digital color effectively, designers need an intuitive way to adjust and manipulate color values. The HSL color model organizes color using three coordinates: Hue, Saturation, and Lightness. Unlike hexadecimal code (#FF0000) or RGB values (255, 0, 0), which represent hardware mixing ratios, HSL describes color in terms that align with human language and design thinking. It is supported natively in modern web browsers and design software, making it a powerful tool for building cohesive color systems and generating dynamic UI variations.",
        "Hue is the color family itself, represented as an angle from 0 to 360 degrees around the color wheel. Red is at 0 degrees, yellow is at 60, green is at 120, cyan is at 180, blue is at 240, and magenta is at 300. By simply changing the hue angle while keeping saturation and lightness constant, a designer can cycle through different colors that share identical visual weight and contrast. This is highly useful for creating themed variations of an interface (like red for errors, green for success, and blue for info).",
        "Saturation is the intensity or purity of the color, ranging from 0% (pure gray, containing no chromatic information) to 100% (the most vivid, saturated version of the selected hue). Lightness ranges from 0% (solid black) to 100% (solid white), with 50% representing the pure, unmixed hue. By adjusting saturation and lightness, designers create variations of a single hue: adding white creates a tint, adding black creates a shade, and adding gray creates a tone. These adjustments are essential for structuring hierarchy and readability.",
        "While HSL is convenient, it has a major limitation: it is not perceptually uniform. Human eyes are not equally sensitive to all wavelengths of light; we perceive yellow as much brighter than blue, even if both have identical HSL lightness values. For example, yellow at `hsl(60, 100%, 50%)` looks blindingly bright, while blue at `hsl(240, 100%, 50%)` looks dark. This means HSL lightness cannot be used to predict contrast or WCAG compliance; designers must measure contrast using relative luminance calculators."
      ],
      keyPoints: [
    ...
    },
    {
      ...
      detailedNotes: [
        "Color branding and trademark law reflect the value of color association. Brands like Tiffany & Co. (Tiffany Blue), T-Mobile (Magenta), and UPS (Pullman Brown) have successfully registered trademarks for their specific brand colors within their industry sectors. This legal protection prevents competitors from using identical colors, recognizing that color is a primary identifier of corporate origin. Establishing a unique, protected brand color requires decades of consistent application across all consumer touchpoints.",
  ...
  review: [
    { question: "How do RGB and CMYK models differ physically and in production?", answer: "RGB is an additive model that mixes emitted light to create lighter colors (for screens); CMYK is a subtractive model that layers physical inks on paper to absorb light and create darker colors (for print)." },
    { question: "Why is HSL useful for editing, and why is it not perceptually uniform?", answer: "HSL provides intuitive controls (Hue, Saturation, Lightness) for creating color variations, but it doesn't account for human eye sensitivity: yellow looks much brighter than blue at the same lightness level." },
    { question: "What is the value of a color harmony scheme, and what are its limitations?", answer: "Harmony schemes (monochromatic, complementary, etc.) provide mathematical starting points for choosing related hues, but designers must still manually adjust value and saturation to ensure readability and legibility." },
    { question: "Why are color psychology claims often oversimplified or inaccurate?", answer: "Color associations are learned, subjective, and highly contextual. They depend on the viewer's culture, personal experiences, the product category, and adjacent elements, rather than being fixed properties of light wavelengths." },
    { question: "How should status information be communicated to ensure accessibility?", answer: "Color can reinforce status, but a redundant cue (like an icon, text label, pattern, or position change) must also convey the status so colorblind and low-vision users can perceive it." },
    { question: "How should a color-vision simulator be used in a design workflow?", answer: "As an approximate diagnostic tool to search for potential color collisions and readability issues. It should complement programmatic contrast checks and user testing rather than replacing them." }
  ],

  quiz: [
    { question: "Which model combines emitted red, green, and blue light for screens?", options: ["CMYK", "RGB", "RYB", "Spot color"], answer: 1, explanation: "RGB is an additive model where red, green, and blue light are combined in varying intensities to display colors on screens." },
    { question: "Which model is used for standard four-color commercial process printing?", options: ["RGB", "HSL", "CMYK", "LAB"], answer: 2, explanation: "CMYK is the process color printing model, using cyan, magenta, yellow, and black inks to print color images." },
    { question: "Why do bright screen colors often shift and look duller when printed?", options: ["Printing presses emit light in opposite directions", "The RGB screen gamut is wider than the CMYK print gamut, meaning some screen colors are out-of-gamut", "CMYK inks are chemically unstable and fade instantly", "All paper absorbs only the green and blue wavelengths"], answer: 1, explanation: "Luminous displays can produce a wider range of color (gamut) than printed inks. Highly saturated screen colors fall 'out of gamut' and print as duller CMYK approximations." },
    { question: "In the HSL model, what does hue represent?", options: ["The opacity of the color layer", "An angle identifying the color family on the color wheel", "The contrast ratio of the color", "The percentage of black ink used"], answer: 1, explanation: "Hue represents the color identity itself, mapped as an angle from 0 to 360 degrees around the color wheel." },
    { question: "What does reducing HSL saturation toward 0% do to a color?", options: ["Turns the color solid black", "Turns the color solid white", "Moves the color toward a neutral gray appearance", "Increases the contrast ratio of the color"], answer: 2, explanation: "Saturation controls chromatic intensity. At 0% saturation, the color loses all chromatic information and becomes a neutral gray." },
    { question: "Which statement about HSL lightness is correct?", options: ["It is perceptually uniform, meaning equal values look equally bright", "It is an editing coordinate and does not map directly to perceived brightness or relative luminance", "It applies only to printed inks and has no effect on screens", "It is measured in pixels per inch"], answer: 1, explanation: "HSL lightness is a mathematical coordinate for editing convenience, not a perceptual measurement. Yellow and blue at 50% lightness appear very different in brightness to the human eye." },
    { question: "Which color harmony scheme uses hues that are directly opposite one another on the color wheel?", options: ["Analogous", "Complementary", "Monochromatic", "Triadic"], answer: 1, explanation: "Complementary colors sit opposite each other on the color wheel, creating maximum visual contrast and energy." },
    { question: "Which scheme varies a single base hue through different saturations and lightnesses?", options: ["Monochromatic", "Triadic", "Complementary", "Analogous"], answer: 0, explanation: "A monochromatic palette is built from a single hue family, utilizing variations in lightness and saturation." },
    { question: "What is the 60-30-10 rule in color design?", options: ["A contrast check ratio for accessibility", "A guideline for color proportions: 60% dominant, 30% secondary, 10% accent", "The ideal angle spacing for a triadic color harmony", "The ratio of RGB subpixels on a standard display screen"], answer: 1, explanation: "The 60-30-10 rule helps designers balance color distribution to establish visual hierarchy and prevent visual competition." },
    { question: "Which statement about color associations is most accurate?", options: ["Colors have fixed psychological meanings that apply to everyone", "Color meanings are learned, subjective, and vary with culture, category, and context", "Warm colors have no impact on heart rate or attention", "Only corporate brands can register color trademarks"], answer: 1, explanation: "Color meanings are not hardwired; they are learned conventions that depend heavily on culture, history, industry category, and surrounding context." },
    { question: "An interface indicates errors only by changing form borders from gray to red. What is the accessibility issue?", options: ["It uses too many colors simultaneously", "Red-green colorblind users may not perceive the color change, meaning color is the sole carrier of information", "Red cannot be displayed on budget screens", "It violates standard CMYK printing guidelines"], answer: 1, explanation: "To be accessible, a design must not rely on color alone to convey status. An error state must pair the color shift with a redundant cue, like a warning icon or text message." },
    { question: "How should a color-vision simulator be used in a design workflow?", options: ["As proof that your text contrast meets WCAG standards", "As an approximate diagnostic aid to check for possible color collisions", "As a replacement for text labels and icons", "To generate automatic color palettes"], answer: 1, explanation: "Simulators help designers identify potential color conflicts where elements blend together for colorblind users, but they are approximate and do not replace contrast checks." },
    { question: "What is a tint in color vocabulary?", options: ["A color mixed with black", "A color mixed with white", "A color mixed with gray", "A color mixed with its complement"], answer: 1, explanation: "A tint is a variation of a color created by adding white, making it lighter and softer." },
    { question: "What is the primary advantage of defining functional color roles (like 'Action-Primary') in a design guide?", options: ["It ensures that the color matches the CMYK press automatically", "It links color application to use cases, making the design system consistent and maintainable", "It reduces the loading time of the web page", "It eliminates the need for HTML heading tags"], answer: 1, explanation: "Functional color roles map colors to their design usage rather than descriptions, ensuring consistency across components and making the system easy to maintain." },
    { question: "According to WCAG AA guidelines, what is the minimum contrast ratio required for normal-sized body text?", options: ["3.0:1", "4.5:1", "7.0:1", "1.5:1"], answer: 1, explanation: "WCAG AA guidelines require a minimum contrast ratio of 4.5:1 for normal-sized body text to ensure readability for users with moderate low vision." }
  ],
